    ...
    public class VegetationSpawnerInspector : Editor
    {
        ...
        private void DrawTrees()
        {
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                for (int i = 0; i < spawner.treeTypes.Count; i++)
                {
                    ...
                    if (GUILayout.Button(new GUIContent("", thumb), (selectedTreeID == i) ? VegetationSpawnerEditor.PreviewTexSelected : VegetationSpawnerEditor.PreviewTex, GUILayout.MaxHeight(thumbSize), GUILayout.Width(thumbSize)))
                    {
                        selectedTreeID = i;
                    }
                }
            ...
            using (var treeChange = new EditorGUI.ChangeCheckScope())
            {
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    if (GUILayout.Button(new GUIContent("Add", EditorGUIUtility.IconContent(EditorGUIUtility.isProSkin ? "d_Toolbar Plus" : "Toolbar Plus").image, "Add new item"), EditorStyles.miniButtonLeft))
                    {
                ...
                if (tree != null)
                {
                    ...
                    using (new EditorGUILayout.HorizontalScope())
                    {
                        ...
                        if (GUILayout.Button(new GUIContent("Add", EditorGUIUtility.IconContent(iconPrefix + "Toolbar Plus").image, "Add new item"), EditorStyles.miniButton))
                        {
        ...
        private void DrawGrass()
        {
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                for (int i = 0; i < spawner.grassPrefabs.Count; i++)
                {
                    ...
                    if (GUILayout.Button(new GUIContent(thumb), (selectedGrassID == i) ? VegetationSpawnerEditor.PreviewTexSelected : VegetationSpawnerEditor.PreviewTex, GUILayout.Height(thumbSize), GUILayout.Width(thumbSize)))
                    {
                       selectedGrassID = i;
                    }
                }
            ...
            using (var grassChange = new EditorGUI.ChangeCheckScope())
            {
                ...
                if (grass != null)
                {
                    ...
                    if (GUILayout.Button(new GUIContent(" Respawn", EditorGUIUtility.IconContent("d_Refresh").image), GUILayout.MaxHeight(30f)))
                    {
                        ...
                        EditorUtility.DisplayProgressBar("Vegetation Spawner", "Respawning grass..", 1f);
                        ...
                        VegetationSpawnerEditor.Log.Add("Respawning grass: " + sw.Elapsed.Seconds + " seconds...");
                    }
                }
                else
                {
        ...
        private void DrawDetailResolutionField()
        {
            ...
            detailResolutionIndex.intValue = EditorGUILayout.Popup(new GUIContent("Grass map resolution", 
                    "Controls the resolution of the internal detail map. A 512px resolution on a 1024x1024 terrain means grass can be spawned 0.5 units apart" +
                    "\n\nHigher resolutions will increase the spawning time"), 
                detailResolutionIndex.intValue, detailResolutions, GUILayout.Width(EditorGUIUtility.labelWidth + 60f));
            ...
            EditorGUILayout.HelpBox("Changing the resolution will clear all grass, and requires all grass to be respawned!", MessageType.Warning);
        }
        ...
        {
            if (spawner.terrains.Count == 0) return EditorGUIUtility.IconContent("d_BuildSettings.Broadcom").image as Texture2D;
        ...
        private void LayerMaskSettings(List<SpawnerBase.TerrainLayerMask> masks)
        {
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                if (GUILayout.Button(new GUIContent("Add", VegetationSpawnerEditor.PlusIcon, "Add new layer mask"), EditorStyles.miniButtonLeft))
                {
                    LayerDropDown(masks);
                }
